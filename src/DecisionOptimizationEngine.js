...
function computeFDSForStrategy(strategy, weights, scenarioParams) {
  ...
  const rel = clamp(
    strategy.reliabilityPct * (scenarioParams.reliabilityModifier ?? 1),
    0,
    100
  );
...
function rankStrategies(strategies, weights, scenarioParams) {
  ...
  const computed = strategies.map(s =>
    computeFDSForStrategy(s, normalized, scenarioParams)
  );
...
function adaptiveLearning(weights, metrics) {
  ...
  const delayThreshold = metrics.delayThreshold || 5;
...
function sensitivityAnalysis(strategies, weights, scenarioParams, perturbations) {
  ...
  const perturbed = rankStrategies(strategies, weights, {
    ...
    costMultiplier:
      (scenarioParams.costMultiplier || 1) *
      (1 + (perturbations.costPct || 0) / 100),
    deliveryMultiplier:
      (scenarioParams.deliveryMultiplier || 1) *
      (1 + (perturbations.trafficPct || 0) / 100),
    reliabilityModifier: scenarioParams.reliabilityModifier || 1,
  });

  const impacts = base.ranked.map(s => {
    const pert = perturbed.ranked.find(p => p.id === s.id);
...
function monteCarloSimulation(
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const kNoise = Math.random() * 0.4 - 0.2;
    ...
    const sc = {
      ...
      costMultiplier:
        (scenarioParams.costMultiplier || 1) * (1 + costNoise),
      deliveryMultiplier:
        (scenarioParams.deliveryMultiplier || 1) * (1 + delayNoise),
    };
  ...
  const stability =
    ((topCounts[baseTopId] || 0) / iterations) * 100;
...
const DecisionOptimizationEngine = {
  normalizeWeights,
  deliveryScore,
  freshnessScore,
  costScore,
...
export default DecisionOptimizationEngine;
